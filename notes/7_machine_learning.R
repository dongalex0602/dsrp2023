...
## look at the rotation of how much of the original variables count
...
## Get rid of NAs
# Only use na.omit after you've selected for the variables you want to include in the model
noNAs <- na.omit(starwars)

# we can use filter to more specifically clean the whole dataset
...
# high correlation?
ggplot(irisAllNumeric, aes(Petal.Length, Sepal.Length)) + 
  geom_point() +
  theme_minimal()

# low correlation?
ggplot(irisAllNumeric, aes(Sepal.Width, Sepal.Length)) +
  geom_point() + 
  theme_minimal()


## Step 4: Perform Feature Selection
# Choose which variables you want to classify/predict
# Choose which variables you want to use as features in your model
# for iris data...
# Classify on Species & Predict on Sepal.Length
...
## Sepal.Length = 2.3 + Petal.Length*0.7967 + Petal.Width*-0.4067 + Species*-0.3312 + Speal.Width*0.5501
...
log_fit <- logistic_reg() %>% 
  ...
  fit(Species ~ Petal.Width + Petal.Length + ., data = binary_train_data) # "+ ." means everything else
...
boost_reg_fit$fit$evaluation_log # tells you the error for each tree
...
f1(class_results$Species == "setosa", class_results$log_pred == "setosa")
